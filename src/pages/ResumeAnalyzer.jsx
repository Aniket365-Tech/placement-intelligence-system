...
import { Upload, FileText, CheckCircle2, AlertCircle, RefreshCw, BarChart3, Fingerprint, X, Download } from 'lucide-react';

const ResumeAnalyzer = () => {
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [analyzed, setAnalyzed] = useState(false);
  const [resumeVersions, setResumeVersions] = useState([
    { id: 1, name: 'Resume_Aniket_Gajbhiye_v2.pdf', date: '2026-02-10', atsScore: 85 },
    { id: 2, name: 'Resume_Aniket_Gajbhiye_v1.pdf', date: '2026-01-15', atsScore: 72 }
  ]);
  ...
  const keywords = {
    found: ['React', 'Python', 'SQL', 'JavaScript', 'Node.js'],
    missing: ['Cloud', 'Docker', 'Kubernetes', 'AWS', 'System Design']
  };
        ...
        <Card className="py-20 flex flex-col items-center justify-center border-dashed border-2 bg-slate-50/50">
          ...
          <div className="flex gap-4">
            <label className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold cursor-pointer hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">
              Select File
              <input type="file" className="hidden" accept=".pdf,.doc,.docx" onChange={handleAnalyze} />
            </label>
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card title="Keyword Match Index" action={<BarChart3 size={18} className="text-indigo-500" />}>
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs font-bold text-slate-400 mb-2">Found Keywords</p>
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-400 mb-2">Missing Keywords</p>
                      <div className="flex flex-wrap gap-2">
                        {keywords.missing.map(keyword => (
                          <Badge key={keyword} variant="error">{keyword}</Badge>
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              <Card title="Resume vs Job Match">
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-bold text-slate-800">Google SDE-1</span>
                      <span className="text-lg font-bold text-emerald-600">92% Match</span>
                    </div>
                    <ProgressBar value={92} color="bg-emerald-500" />
                  </div>
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-bold text-slate-800">Microsoft SWE</span>
                      <span className="text-lg font-bold text-blue-600">85% Match</span>
                    </div>
                    <ProgressBar value={85} color="bg-blue-500" />
                  </div>
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-bold text-slate-800">Amazon SDE-1</span>
                      <span className="text-lg font-bold text-amber-600">78% Match</span>
                    </div>
              ...
              <Card title="Section Feedback">
                <div className="space-y-3">
                  {[
                    { section: 'Education', score: 95, feedback: 'Well formatted with CGPA highlighted' },
                    { section: 'Experience', score: 88, feedback: 'Good use of action verbs and metrics' },
                    { section: 'Skills', score: 75, feedback: 'Consider adding proficiency levels' },
                    { section: 'Projects', score: 82, feedback: 'Add more technical details and impact' }
                  ].map((item, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-sm font-bold text-slate-800">{item.section}</span>
              ...
              <Card title="Improvement Checklist">
                <div className="space-y-4">
                  {[
                    ...
                    { text: "Add volunteer experience section", done: false },
                    { text: "Include GitHub and LinkedIn links", done: true }
                  ].map((item, idx) => (
          ...
          <Card title="Resume Version Manager">
            <div className="space-y-3">
              {resumeVersions.map(version => (
                <div key={version.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="flex items-center gap-4">
                    <FileText className="text-blue-600" size={24} />
                    <div>
                      <p className="text-sm font-bold text-slate-800">{version.name}</p>
                      <p className="text-xs text-slate-500">Uploaded: {version.date}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="text-sm font-bold text-blue-600">{version.atsScore}%</p>
              ...
              <button className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 hover:border-blue-500 hover:text-blue-600 transition-colors font-medium">
                + Upload New Version
              </button>
            </div>
          </Card>
