...
import { Search, Filter, Briefcase, MapPin, DollarSign, Calendar, Flame, Target, Users } from 'lucide-react';
        ...
        <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-slate-200">
          {['All', 'Product', 'Service', 'Fintech'].map(cat => (
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${filter === cat ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
            >
              {cat}
            </button>
          ))}
            ...
            <Card title="Quick Filters">
               <div className="space-y-6">
                  <div>
                    ...
                    <input type="range" className="w-full accent-blue-600" min="3" max="50" />
               ...
               {jobsList.map(job => (
                 <Card key={job.id} className="group">
                    <div className="flex justify-between items-start mb-4">
                       ...
                       <Badge variant={job.competitionLevel === 'High' ? 'error' : 'success'} className="flex items-center gap-1">
                    ...
                    <div className="flex gap-3">
                       <button className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-100">Apply Now</button>
